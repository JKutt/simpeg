...
# from SimPEG.utils.solver_utils import SolverMUMPS as Solver
import time
...
def run():
    ...
    ct = time.time()
    patch = DCIP.loadDias(r"Combo_200-1200mDP.DAT")
    patch.readings = patch.readings[:-1]
    ...
    topo = np.loadtxt(r"Topo.txt", delimiter=" ")
    ...
    # strt_mesh = time()
    global_mesh = discretize.TensorMesh.readUBC(r"Fine.msh")
    # print('time to load mesh: ', time() - strt_mesh )
    ...
    for ii, source in enumerate(survey.source_list):
        ...
        if cnt == 14 or ii == len(survey.source_list)-1:
            simlite = {}
            simlite["dataset"] = "/home/juan/dev/Combo_200-1200mDP.DAT"
            simlite["activecells"] = "/home/juan/dev/active_cells.npy"
            simlite["mesh"] = "/home/juan/dev/Fine.msh"

            src_collect.append(source)        
            idx_end = idx_end + source.receiver_list[0].nD
            simlite["sourcefrom"] = cnt_prev
            simlite["sourceto"] = ii
            simlite["model"] = "/home/juan/dev/model.npy"
            lite_sims.append(simlite)
            cnt_prev = ii
    ...
    global_misfit.lite_simulations = lite_sims
    ...
    # outputs = invProb.get_dpred(m0_dc, f=None)
    outputs = invProb.get_fields(m0_dc)
